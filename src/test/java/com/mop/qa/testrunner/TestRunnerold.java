...
public class TestRunnerold {
	...
	public void test() {
		...
		try {
			String log4jConfPath = "/Users/557743/Desktop/USA/log4j.properties";
	 ...
	 * String platFormName = null; List<List<TestParameter1>> testRunner = new
	 ...
	 * ArrayList<TestParameter1>(); for (int count = 1; count <=
	 * sheet.getLastRowNum(); count++) { TestParameter1 test = new
	 * TestParameter1(); HSSFRow row = sheet.getRow(count); int count1 =
	 * Integer.parseInt((row.getCell(4).toString())); if
	 * (row.getCell(2).toString().equalsIgnoreCase("Yes")) { for (int l = 1; l
	 * <= count1; l++) { String name = row.getCell(0).toString(); test = new
	 * TestParameter1(); test.setTestName(name + "-Run" + l);
	 * test.setTestClass(row.getCell(1).toString());
	 * test.setPlatFormName(row.getCell(5).toString());
	 * test.setUdid(row.getCell(6).toString()); list.add(test); } } }
	 * testRunner.add(list); } return testRunner; }
	 */
	public static List<List<TestParameter1>> testList() throws Exception {
